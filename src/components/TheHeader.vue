  ...
  <div class="min-h-full">
          ...
          <div class="flex items-center">
            ...
            <div class="ml-10 flex items-baseline space-x-4">
              <a v-for="(item, index) in navigation" :key="item.name" :href="item.href" 
                ...
                :class="{ 'text-red': item.current, 'text-white': !item.current }"
      ...
      <DisclosurePanel class="md:hidden">
        <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">
          <DisclosureButton 
            ...
            as="a" :href="item.href" 
            :class="[item.current ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white', 'block rounded-md px-3 py-2 text-base font-medium']" :aria-current="item.current ? 'page' : null">{{ item.name }}</DisclosureButton>
        </div>
    ...
    <header class="bg-white shadow">
      <div class="mx-auto max-w-7xl py-6 px-4 sm:px-6 lg:px-8">
        <h1 
        :onclick="item.href"
        v-for="item in navigation" :key="item.name" v-show="item.current == true" class="text-3xl font-bold tracking-tight text-gray-900">{{ item.name }}</h1>
      </div>
    </header>
    <main>
